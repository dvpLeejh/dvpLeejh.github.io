...
차원 축소의 각 커널기법을 활용하여 데이터를 전처리한 후
분류모델을 통해 각 커널의 성능 확인하기
...
먼저 커널을 시각적으로 표시하기 위해 plot_digits 도구를 만든다.
...
위의 그림들은 각 커널들의 전처리 결과를 시각적으로 표현한 것이다.

이제 전치리가 완료된 데이터를 가지고 RandomForestClassifier을 활용하여

분류를 진행한뒤, 정확도에 따라 커널에 따라 전처리가 얼마나 잘 되었는지
